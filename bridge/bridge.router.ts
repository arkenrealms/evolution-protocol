...
export const createRouter = (service: Service) => {
  return router({
    // @ts-ignore
    connect: procedure.input(Schema.AnyInput).mutation(({ input, ctx }) => (service.connect as any)(input, ctx)),

    // @ts-ignore
    disconnect: procedure.input(Schema.AnyInput).mutation(({ input, ctx }) => (service.disconnect as any)(input, ctx)),
